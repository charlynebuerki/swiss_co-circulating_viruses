# Nextstrain ReVSeq repository for influenza A/H1N1, A/H3N2, B/Victoria builds 

This repository provides the pipeline to run the Nextstrain analysis of the3 types of influenza. 
...
You must first acquire the reference genomes in ingest/data/{pathogen}/. You also need the annotated gff3 file of that genome. To create it, you can run this script: [generate_from_genbank.py](bin/generate_from_genbank.py) manually (i.e from a terminal launch python3 ingest/bin/generate_from_genbank.py --reference "NC_003461" --output-dir "ingest/data/pathogen_name" ) with arguments (1) the genbank reference of the sequence and (2) the output directory. The outputs should be stored in ingest/data/{pathogen}. You can find the genbank references in ingest/defaults/config.yaml under accession_reference. 
...
- you must also download the influenza data (global data). You can do this using the Locolus API by filtering for individual segments. 
